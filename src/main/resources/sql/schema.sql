...
CREATE TABLE NATION (
     ...
     CONTINENT_CODE VARCHAR(10) COMMENT '대륙코드',
...
-- 목적지 정보(국가단위가 기본이나 예외적으로 하와이, 유럽 등이 있음.)
DROP TABLE IF EXISTS ARRIVAL;
CREATE TABLE ARRIVAL (
    ARRIVAL_CODE CHAR(2) NOT NULL COMMENT '목적지코드',
    ARRIVAL_NAME VARCHAR(50) NOT NULL COMMENT '목적지명',
    MENU_YN CHAR(1) DEFAULT 'N' COMMENT '메뉴사용유무',
    ORDINAL_POSITION INT COMMENT '순서',
    USE_YN CHAR(1) DEFAULT 'N' COMMENT '사용유무',
    CREATE_AT DATETIME DEFAULT NOW() COMMENT '작성일자',
    UPDATE_AT DATETIME COMMENT '변경일자',
    CONSTRAINT PK_ARRIVAL PRIMARY KEY (ARRIVAL_CODE)
);

-- 지역정보
DROP TABLE IF EXISTS AREA;
CREATE TABLE AREA (
    AREA_CODE CHAR(2) NOT NULL COMMENT '지역코드',
    AREA_NAME CHAR(2) NOT NULL COMMENT '지역명',
    ARRIVAL_CODE CHAR(2) COMMENT '목적지코드',
    ...
    CONSTRAINT FK_AREA_1 FOREIGN KEY (ARRIVAL_CODE) REFERENCES ARRIVAL (ARRIVAL_CODE),
    CONSTRAINT FK_AREA_2 FOREIGN KEY (NATION_CODE) REFERENCES NATION (NATION_CODE)
);
...
CREATE TABLE PRODUCT (
    ...
    CONTENT LONGTEXT COMMENT '상품내용',
    DESCRIPTION VARCHAR(500) COMMENT '요약설명(비고)',
    WRITER VARCHAR(20) COMMENT '작성자',
    REP_IMAGE_SRC VARCHAR(1000) COMMENT '대표이미지 URL',
    REP_IMAGE_PATH VARCHAR(1000) COMMENT '대표이미지저장경로',
    USE_YN CHAR(1) DEFAULT 'Y' COMMENT '사용유무',
    CREATE_AT DATETIME DEFAULT NOW() COMMENT '작성일자',
    UPDATE_AT DATETIME COMMENT '변경일자',
...
CREATE TABLE PRODUCT_IMAGE (
    ...
    IMAGE_SRC VARCHAR(1000) COMMENT '이미지 SRC',
    IMAGE_PATH VARCHAR(1000) COMMENT '이미지저장경로',
    TITLE VARCHAR(100) COMMENT '제목',
...
DROP TABLE IF EXISTS VIEW_COMPONENT_EDITOR;
CREATE TABLE VIEW_COMPONENT_EDITOR (
    VIEW_COMPONENT_ID BIGINT NOT NULL COMMENT '화면요소 ID',
    CONTENT LONGTEXT COMMENT '내용',
    CREATE_AT DATETIME DEFAULT NOW() COMMENT '작성일자',
    UPDATE_AT DATETIME COMMENT '변경일자',
    CONSTRAINT PK_VIEW_COMPONENT_EDITOR PRIMARY KEY (VIEW_COMPONENT_ID),
    CONSTRAINT FK_VIEW_COMPONENT_EDITOR FOREIGN KEY (VIEW_COMPONENT_ID) REFERENCES VIEW_COMPONENT (VIEW_COMPONENT_ID)
);
...
CREATE TABLE CAROUSEL (
    ...
    TITLE VARCHAR(100) COMMENT '제목',
    IMAGE_PATH VARCHAR(2000) COMMENT '이미지경로',
